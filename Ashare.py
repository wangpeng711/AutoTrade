...
def get_price_day_tx(code, end_date='', count=10, frequency='1d'):  # 日线获取
    ...
    # 创建空的 DataFrame，包含所有可能的字段
    df = pd.DataFrame(columns=['time', 'open', 'close', 'high', 'low', 'volume'])
    if max_fields == 6:  # 假设有的对象只有6个字段
        df = pd.DataFrame(buf,columns=['time', 'open', 'close', 'high', 'low', 'volume'])
    elif max_fields == 7:  # 假设有的对象有7个字段
        df = pd.DataFrame(buf,columns=['time', 'open', 'close', 'high', 'low', 'volume', 'fh'])
    # 如果有其他字段数的情况，可以继续添加对应的处理逻辑
...
def get_stock_list(page,num):
    data_json_e = ""
    try:
        """获取A股所有股票代码"""
        url = f'http://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php/Market_Center.getHQNodeData?page={page}&num={num}&sort=symbol&asc=1&node=hs_a&symbol=&_s_r_a=init'
        r = requests.get(url)
        data_text = r.text
        data_json_e = data_text
        data_json = json.loads(data_text)
        df = pd.DataFrame(data_json, columns=['symbol', 'code', 'name'])
        return df
    except json.JSONDecodeError as e:
        print(f"解析股票列表时发生错误： {e}")
        print(f"text: {data_json_e}")
        return []
    except Exception as e:
        print(f"获取股票列表时发生未知错误： {e}")
        return []
